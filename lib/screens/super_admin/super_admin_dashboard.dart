...
import 'package:foodfleet/screens/super_admin/settings.dart';
...
class _SuperAdminDashboardState extends State<SuperAdminDashboard> {
  ...
  /// ✅ One-time migration — adds default categories to existing restaurants
  /// Safe to run multiple times — skips restaurants that already have them
  Future<void> _runMigration() async {
    try {
      await DatabaseService().migrateDefaultCategories();
    } catch (e) {
      ...
      drawer: Drawer(
        ...
        child: ListView(
          ...
          children: [
              ...
              child: Column(
                ...
                children: [
                  Icon(Icons.admin_panel_settings,
                      color: colors.onSecondary, size: 40),
            ...
            _buildDrawerItem(
                Icons.restaurant, "Manage Restaurants (Admins)", 1, colors),
  ...
  Widget _buildDrawerItem(
      IconData icon, String title, int index, ColorScheme colors) {
    final bool selected = _selectedIndex == index;
    return ListTile(
      leading:
          Icon(icon, color: selected ? colors.onSecondary : colors.tertiary),
      title: Text(
        title,
        style: TextStyle(
