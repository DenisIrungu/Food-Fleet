  ...
  Future<void> _upload(BuildContext context) async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        final url = await db.updateProfilePictureWeb(userData.uid, bytes);
        if (url != null) {
          await db.updateRestaurantData(restaurant.id, {'imageUrl': url});
        }
      } else {
        final url = await db.updateProfilePicture(
          userData.uid,
          File(picked.path),
        );
        if (url != null) {
